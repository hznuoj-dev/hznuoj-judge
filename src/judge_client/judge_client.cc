...
#define MOUNT_RO(path) \
  { execute_cmd("/bin/mount -o bind,sync,ro /" path " " path); }

#define MOUNT_RW(path) \
  { execute_cmd("/bin/mount -o bind,sync,rw /" path " " path); }

#define UMOUNT(path)                    \
  {                                     \
    if (access(path, F_OK) == 0) {      \
      execute_cmd("/bin/umount " path); \
      execute_cmd("/bin/rmdir " path);  \
    }                                   \
  }
...
void init_syscalls_limits(int lang) {
  ...
  if (DEBUG) {
    LOG_INFO("init_call_counter:%d", lang);
  }
...
void make_diff_out_full(FILE *f1, FILE *f2, int c1, int c2, const char *path) {
  execute_cmd("echo '========Failed test [%s]========='>>diff.out",
...
void make_diff_out_simple(FILE *f1, FILE *f2, int c1, int c2,
                          const char *path) {
  execute_cmd("echo '========Failed test [%s]========='>>diff.out",
              getFileNameFromPath(path));
...
int compare(const char *file1, const char *file2) {
  ...
  if (!(f1 = fopen(file1, "re"))) return OJ_AC;
  for (p1 = s1; EOF != fscanf(f1, "%s", p1);)
    while (*p1) p1++;
  fclose(f1);
  if (!(f2 = fopen(file2, "re"))) return OJ_RE;
...
bool check_login() {
  const char *cmd =
      " wget --post-data=\"checklogin=1\" --load-cookies=cookie "
...
void login() {
  if (!check_login()) {
    const char *cmd =
        "wget --post-data=\"user_id=%s&password=%s\" --load-cookies=cookie "
        "--save-cookies=cookie --keep-session-cookies -q -O - \"%s/login.php\"";
    FILE *fjobs =
        read_cmd_output(cmd, http_username, http_password, http_baseurl);
    // fscanf(fjobs,"%d",&ret);
    pclose(fjobs);
  }
}
...
void _update_solution_mysql(int solution_id, int result, int time, int memory,
  ...
  if (oi_mode) {
    sprintf(sql,
            "UPDATE %s SET "
            "result=%d,time=%d,memory=%d,pass_rate=%f,judger='%s',judgetime="
            "now() WHERE solution_id=%d LIMIT 1%c",
            tbname, result, time, memory, pass_rate, http_username, solution_id,
            0);
  } else {
    sprintf(
        sql,
        "UPDATE %s SET result=%d,time=%d,memory=%d,judger='%s',judgetime=now() "
        "WHERE solution_id=%d LIMIT 1%c",
  ...
  if (mysql_real_query(conn, sql, strlen(sql))) {
    if (DEBUG) {
      LOG_INFO("..update failed! %s\n", mysql_error(conn));
    }
  }

  if (sim) {
    sprintf(sql,
            "insert into sim(s_id,sim_s_id,sim) values(%d,%d,%d) on duplicate "
            "key update  sim_s_id=%d,sim=%d",
...
void _update_solution_http(int solution_id, int result, int time, int memory,
  ...
  const char *cmd =
      " wget "
      "--post-data=\"update_solution=1&sid=%d&result=%d&time=%d&memory=%d&sim=%"
      "d&simid=%d&pass_rate=%f\" --load-cookies=cookie --save-cookies=cookie "
      "--keep-session-cookies -q -O - \"%s/admin/problem_judge.php\"";
...
void _addceinfo_http(int solution_id) {
  ...
  const char *cmd =
      " wget --post-file=\"ce.post\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O - "
      "\"%s/admin/problem_judge.php\"";
  FILE *fjobs = read_cmd_output(cmd, http_baseurl);
  // fscanf(fjobs,"%d",&ret);
...
void _addreinfo_http(int solution_id, const char *filename) {
  ...
  const char *cmd =
      " wget --post-file=\"re.post\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O - "
      "\"%s/admin/problem_judge.php\"";
  FILE *fjobs = read_cmd_output(cmd, http_baseurl);
...
void _update_user_mysql(char *user_id) {
  ...
  sprintf(sql,
          "UPDATE `users` SET `solved`=(SELECT count(DISTINCT `problem_id`) "
          "FROM `solution` WHERE `user_id`=\'%s\' AND `result`=\'4\') WHERE "
          "`user_id`=\'%s\'",
          user_id, user_id);
  ...
  sprintf(sql,
          "UPDATE `users` SET `submit`=(SELECT count(*) FROM `solution` WHERE "
          "`user_id`=\'%s\' and problem_id>0) WHERE `user_id`=\'%s\'",
          user_id, user_id);
...
void _update_user_http(char *user_id) {
  const char *cmd =
      " wget --post-data=\"updateuser=1&user_id=%s\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O - "
      "\"%s/admin/problem_judge.php\"";
...
void _update_problem_http(int pid) {
  const char *cmd =
      " wget --post-data=\"updateproblem=1&pid=%d\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O - "
      "\"%s/admin/problem_judge.php\"";
...
void _update_problem_mysql(int p_id) {
  ...
  sprintf(
      sql,
      "UPDATE `problem` SET `accepted`=(SELECT count(*) FROM `solution` WHERE "
      "`problem_id`=\'%d\' AND `result`=\'4\') WHERE `problem_id`=\'%d\'",
      p_id, p_id);
  ...
  sprintf(sql,
          ...
          "WHERE `problem_id`=\'%d\') WHERE `problem_id`=\'%d\'",
          p_id, p_id);
...
int compile(int lang, char *work_dir) {
  ...
  const char *CP_X[] = {"g++", "-fno-asm", "-Wall",      "-O2",
                        "-lm", "--static", "-std=c++11", "-DONLINE_JUDGE",
                        "-o",  "Main",     "Main.cc",    NULL};
  ...
  if (pid == 0) {
    ...
    if (compile_chroot && lang != 3 && lang != 9 && lang != 6 && lang != 18 &&
      ...
      if (access("usr", F_OK) == -1) {
        execute_cmd(
            "mkdir -p root/.cache/go-build tmp usr bin lib lib32 libx32 etc "
            "proc");
        ...
        if (lang > 2 && lang < 19 && lang != 13 && lang != 14 && lang != 6 &&
            lang != 18 && lang != 10) {
          execute_cmd("mkdir -p dev");
          MOUNT_RO("dev");
        }

#ifndef __i386__
        execute_cmd("mkdir -p lib64");
        MOUNT_RO("lib64");
#endif
      }

      if (chroot(work_dir)) {
        LOG_ERROR("warning chroot fail!\n");
        return 1;
      }

      LOG_INFO("end mount compile file");
    }
...
void _get_solution_http(int solution_id, char *work_dir, int lang) {
  ...
  const char *cmd2 =
      "wget --post-data=\"getsolution=1&sid=%d\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O %s "
      "\"%s/admin/problem_judge.php\"";
...
void _get_custominput_http(int solution_id, char *work_dir) {
  ...
  const char *cmd2 =
      "wget --post-data=\"getcustominput=1&sid=%d\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O %s "
      "\"%s/admin/problem_judge.php\"";
...
void _get_solution_info_mysql(int solution_id, int &p_id, char *user_id,
  ...
  if (turbo_mode == 2) {
    ...
    sprintf(sql,
            "SELECT problem_id, user_id, language FROM solution2 where "
            "solution_id=%d",
            solution_id);
    ...
    sprintf(sql,
            "SELECT problem_id, user_id, language FROM solution where "
            "solution_id=%d",
            solution_id);
  }
...
void _get_solution_info_http(int solution_id, int &p_id, char *user_id,
  ...
  const char *cmd =
      "wget --post-data=\"getsolutioninfo=1&sid=%d\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O - "
      "\"%s/admin/problem_judge.php\"";
...
void _get_problem_info_http(int p_id, int &time_lmt, int &mem_lmt, int &isspj) {
  ...
  const char *cmd =
      "wget --post-data=\"getprobleminfo=1&pid=%d\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O - "
      "\"%s/admin/problem_judge.php\"";
...
#ifdef __x86_64__
  execute_cmd("/bin/cp -a /usr/lib/x86_64-linux-gnu/libruby* %s/usr/lib/",
              work_dir);
...
#ifdef __x86_64__
  execute_cmd("/bin/cp -a /usr/lib/x86_64-linux-gnu/libguile* %s/usr/lib/",
              work_dir);
  execute_cmd("/bin/cp -a /usr/lib/x86_64-linux-gnu/libgc* %s/usr/lib/",
              work_dir);
  execute_cmd("/bin/cp -a /usr/lib/x86_64-linux-gnu/libffi* %s/usr/lib/",
              work_dir);
...
int copy_php_runtime(char *work_dir) {
  ...
  execute_cmd("/bin/cp /usr/lib/x86_64-linux-gnu/libxml2.so* %s/usr/lib/",
              work_dir);
  execute_cmd("/bin/cp /usr/lib/x86_64-linux-gnu/libicuuc.so* %s/usr/lib/",
              work_dir);
  execute_cmd("/bin/cp /usr/lib/x86_64-linux-gnu/libicudata.so* %s/usr/lib/",
              work_dir);
  execute_cmd("/bin/cp /usr/lib/x86_64-linux-gnu/libstdc++.so* %s/usr/lib/",
              work_dir);
  execute_cmd("/bin/cp /usr/lib/x86_64-linux-gnu/libssl* %s/usr/lib/",
              work_dir);
  execute_cmd("/bin/cp /usr/lib/x86_64-linux-gnu/libcrypto* %s/usr/lib/",
              work_dir);
#endif
...
int copy_mono_runtime(char *work_dir) {
  ...
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libm.so.6 "
      "%s/usr/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/librt.so.1 "
      "%s/usr/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libpthread.so.0 "
      "%s/usr/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libgcc_s.so.1 "
      "%s/usr/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libc.so.6 "
      "%s/usr/lib/x86_64-linux-gnu/",
      work_dir);
...
int copy_js_runtime(char *work_dir) {
  ...
  execute_cmd("/bin/mkdir -p %s/usr/lib %s/lib/i386-linux-gnu/ %s/lib64/",
              work_dir, work_dir, work_dir);
  execute_cmd("/bin/cp /lib/i386-linux-gnu/libz.so.*  %s/lib/i386-linux-gnu/",
              work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/i386-linux-gnu/libuv.so.*  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/i386-linux-gnu/libicui18n.so.*  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/i386-linux-gnu/libicuuc.so.*  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/i386-linux-gnu/libicudata.so.*  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/i386-linux-gnu/libtinfo.so.*  %s/lib/i386-linux-gnu/",
      work_dir);

  execute_cmd(
      "/bin/cp /usr/lib/i386-linux-gnu/libcares.so.*  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd("/bin/cp /usr/lib/libv8.so.*  %s/lib/i386-linux-gnu/", work_dir);
  execute_cmd("/bin/cp /lib/i386-linux-gnu/libssl.so.*  %s/lib/i386-linux-gnu/",
              work_dir);
  execute_cmd(
      "/bin/cp /lib/i386-linux-gnu/libcrypto.so.*  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd("/bin/cp /lib/i386-linux-gnu/libdl.so.*  %s/lib/i386-linux-gnu/",
              work_dir);
  execute_cmd("/bin/cp /lib/i386-linux-gnu/librt.so.*  %s/lib/i386-linux-gnu/",
              work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/i386-linux-gnu/libstdc++.so.*  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/i386-linux-gnu/libpthread.so.*  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd("/bin/cp /lib/i386-linux-gnu/libc.so.6  %s/lib/i386-linux-gnu/",
              work_dir);
  execute_cmd("/bin/cp /lib/i386-linux-gnu/libm.so.6  %s/lib/i386-linux-gnu/",
              work_dir);
  execute_cmd(
      "/bin/cp /lib/i386-linux-gnu/libgcc_s.so.1  %s/lib/i386-linux-gnu/",
      work_dir);
  execute_cmd("/bin/cp /lib/ld-linux.so.*  %s/lib/", work_dir);

#ifdef __x86_64__
  execute_cmd(
      "/bin/mkdir -p %s/usr/lib/x86_64-linux-gnu/ %s/lib/x86_64-linux-gnu/",
      work_dir, work_dir);

  // execute_cmd("/bin/cp /usr/lib/x86_64-linux-gnu/
  // %s/usr/lib/x86_64-linux-gnu/", work_dir);
  execute_cmd("/bin/cp /usr/lib/libv8.so.*  %s/usr/lib/", work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libcares.so.* "
      "%s/usr/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/libz.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libuv.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/librt.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/libpthread.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/libdl.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/libssl.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/libcrypto.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libicui18n.so.* "
      "%s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libicuuc.so.* "
      "%s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libstdc++.so.* "
      "%s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/libm.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/libgcc_s.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd(
      "/bin/cp /lib/x86_64-linux-gnu/libc.so.* %s/lib/x86_64-linux-gnu/",
      work_dir);
  execute_cmd("/bin/cp /lib64/ld-linux-x86-64.so.* %s/lib64/", work_dir);
  execute_cmd(
      "/bin/cp /usr/lib/x86_64-linux-gnu/libicudata.so.* "
      "%s/lib/x86_64-linux-gnu/",
      work_dir);
#endif
...
void run_solution(int &lang, char *work_dir, int &time_lmt, int &usedtime,
  ...
  if (DEBUG) {
    LOG_INFO("LIM_CPU=%d", (int)(LIM.rlim_cur));
  }
...
int fix_java_mis_judge(char *work_dir, int &ACflg, int &topmemory,
  ...
  comp_res =
      execute_cmd("/bin/grep 'Could not create'  %s/error.out", work_dir);
...
void watch_solution(pid_t pidApp, char *infile, int &ACflg, int isspj,
  ...
  while (1) {
    ...
    if (lang == 3 || lang == 7 || lang == 9 || lang == 13 || lang == 14 ||
        lang == 16 || lang == 17) {
    ...
    if ((lang < 4 || lang >= 19 || lang == 13 || lang == 14 || lang == 5 ||
         lang == 9 || lang == 6 || lang == 18) &&
    ...
    /*
      ...
      WIFSTOPPED: return true if the process is paused or stopped while ptrace
      is watching on it WSTOPSIG: get the signal if it was stopped by signal
    */
...
int get_test_file(char *work_dir, int p_id) {
  ...
  const char *cmd =
      " wget --post-data=\"gettestdatalist=1&pid=%d\" --load-cookies=cookie "
      "--save-cookies=cookie --keep-session-cookies -q -O - "
      "\"%s/admin/problem_judge.php\"";
  ...
  while (fgets(filename, BUFFER_SIZE - 1, fjobs) != NULL) {
    ...
    const char *check_file_cmd =
        " wget --post-data=\"gettestdatadate=1&filename=%d/%s\" "
    ...
    if (access(localfile, 0) == -1 || local_date < remote_date) {
      ...
      const char *cmd2 =
          " wget --post-data=\"gettestdata=1&filename=%d/%s\" "
          "--load-cookies=cookie --save-cookies=cookie --keep-session-cookies "
          "-q -O \"%s\"  \"%s/admin/problem_judge.php\"";
...
int main(int argc, char **argv) {
  ...
  if (lang >= 3 && lang <= 17 && lang != 10 && lang != 13 && lang != 14) {
    // the limit for java
...
#ifdef _mysql_h
    if (!http_judge) mysql_close(conn);
#endif
  ...
  for (; (oi_mode || ACflg == OJ_AC || ACflg == OJ_PE) &&
         (dirp = readdir(dp)) != NULL;) {
    // check if the file is *.in or not
    namelen = isInFile(dirp->d_name);
    if (namelen == 0) continue;
    if (http_judge && (!data_list_has(dirp->d_name))) continue;
